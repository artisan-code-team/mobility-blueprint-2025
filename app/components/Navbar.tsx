...
const sections = [
  { id: "lateral-lines", title: "Lateral Lines" },
  ...
  return (
    <div ref={navBarRef} className="sticky top-0 z-50">
            ...
            >
              {!open && (
                <>
                  <span
                    aria-hidden="true"
                    className="font-mono text-sm text-blue-900"
                  >
                    {(mobileActiveIndex + 1).toString().padStart(2, "0")}
                  </span>
                  <span className="ml-4 text-base font-medium text-slate-800">
              ...
              >
                ...
                <MenuIcon open={open} className="h-6 w-6 stroke-slate-800" />
              </Popover.Button>
            </div>
      ...
      <div className="hidden sm:flex sm:h-32 sm:justify-center sm:border-b sm:border-slate-200 sm:bg-white/95 sm:[@supports(backdrop-filter:blur(0))]:bg-white/80 sm:[@supports(backdrop-filter:blur(0))]:backdrop-blur">
        <ol
          role="list"
          className="mb-[-2px] grid auto-cols-[minmax(0,15rem)] grid-flow-col text-base font-medium text-slate-800 [counter-reset:section]"
        >
          {sections.map((section, sectionIndex) => (
              ...
              <Link
                ...
                className={clsx(
                  ...
                  sectionIndex === activeIndex
                    ? "border-blue-900 bg-blue-50 text-blue-900 before:text-blue-900"
                    : "border-transparent before:text-slate-500 hover:bg-blue-50/40 hover:before:text-slate-800"
                )}
              >
                {section.title}
              </Link>
